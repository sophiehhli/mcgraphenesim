...
def diffuse_generation(specie, n, intersection, fermi_circle): 
	newdir = diffuse_newdir(n)
	if specie == 'phonon': 
		newparticle = point.Phonon(intersection.coords, newdir)
	elif specie == 'electron': 
		k_vector_index = fermi_circle.pick_vector_index()
		newparticle = point.Electron(intersection.coords, newdir, fermi_circle, k_vector_index)
	return newparticle
...
def intialize_particle(lead, specie, fermi_circle='null'): 
	random_point = gen_line_point(lead.start, lead.end)
	return diffuse_generation(specie, lead.normal, random_point, fermi_circle)
